export class Jungle {
  ...
  constructor(context: BaseAudioContext) {
    ...
    this.shiftDownBuffer = this.createDelayTimeBuffer(this.bufferTime, this.fadeTime, false);
    this.shiftUpBuffer = this.createDelayTimeBuffer(this.bufferTime, this.fadeTime, true);
    ...
    const fadeBuffer = this.createFadeBuffer(this.bufferTime, this.fadeTime);
    ...
    // Start
    const t = context.currentTime + 0.050; // offset slightly
  ...
  createFadeBuffer(activeTime: number, fadeTime: number) {
  ...
  createDelayTimeBuffer(activeTime: number, fadeTime: number, shiftUp: boolean) {
    const context = this.context;
    const length1 = activeTime * context.sampleRate;
    const length2 = (activeTime - 2*fadeTime) * context.sampleRate;
    const length = length1 + length2;
  ...
  setDelay(delayTime: number) {
    // using setTargetAtTime directly fails on OfflineAudioContext if not using correct setup,
    // we just fallback to value setting or simple automation.
    const time = this.context.currentTime;
    this.modGain1.gain.setTargetAtTime(0.5*delayTime, time, 0.010);
    this.modGain2.gain.setTargetAtTime(0.5*delayTime, time, 0.010);
  }
